...
public class Earth extends JPanel implements ActionListener {
    ...
    public Earth(int width, int height) {
        ...
        try {
            sunImg = ImageIO.read(new File("src/gui3_2_4/sun.png"));

            // Растояние от Солнца, скорость и размеры не в 100% масштабе, но пропорциональны
            planets.add(new Planet("src/gui3_2_4/mercury.png", 50, 0.05, 10));   // Меркурий
            planets.add(new Planet("src/gui3_2_4/venus.png",   80, 0.035, 16));  // Венера
            planets.add(new Planet("src/gui3_2_4/earth.png",  110, 0.03, 18));   // Земля
            planets.add(new Planet("src/gui3_2_4/mars.png",   140, 0.024, 16));  // Марс
            planets.add(new Planet("src/gui3_2_4/jupiter.png",200, 0.013, 28));  // Юпитер
            planets.add(new Planet("src/gui3_2_4/saturn.png", 260, 0.01, 26));   // Сатурн
            planets.add(new Planet("src/gui3_2_4/uranus.png", 310, 0.007, 22));  // Уран
            planets.add(new Planet("src/gui3_2_4/neptune.png",360, 0.005, 22));  // Нептун
        } catch (IOException e) {
            e.printStackTrace();
        }
